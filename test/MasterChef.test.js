...
contract('MasterChef', ([alice, bob, carol, referrer, treasury, dev, fee, owner]) => {
    beforeEach(async () => {
        ...
        this.referral = await FlokiReferral.new({ from: owner });
        this.chef = await MasterChef.new(this.floki.address, '100', '1000', { from: owner });

        await this.floki.transferOwnership(this.chef.address, { from: owner });
        await this.referral.updateOperator(this.chef.address, true, { from: owner });
        await this.chef.setFlokiReferral(this.referral.address, { from: owner });
