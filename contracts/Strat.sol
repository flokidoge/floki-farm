...
contract Strat is Ownable, ReentrancyGuard, Pausable {
    ...
    function deposit(address _tokenAddress, uint256 _wantAmt)
        public
        virtual
        nonReentrant
        whenNotPaused
        returns (uint256)
    {
        ...
        function withdraw(address _tokenAddress, uint256 _wantAmt)
        public
        virtual
        nonReentrant
        returns (uint256)
    {
        ...
        uint256 wantAmt = IBEP20(_tokenAddress).balanceOf(address(this));
        if (_wantAmt > wantAmt) {
            _wantAmt = wantAmt;
        }

        IBEP20(_tokenAddress).safeTransfer(
            address(msg.sender),
            _wantAmt
        );
